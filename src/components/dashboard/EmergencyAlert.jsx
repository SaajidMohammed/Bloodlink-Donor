...
import { MapPin, Droplet, Clock, Building2, User, Loader2, ChevronRight, Navigation } from 'lucide-react';
import NavigateModal from './NavigateModal';

const EmergencyAlert = ({ request, onRespond, responding }) => {
  const [showNavigate, setShowNavigate] = useState(false);

  const isResponded = request?.has_responded ||
      ...
      <div className={`bg-white border ${urgencyStyles.border} shadow-sm rounded-2xl p-5 hover:shadow-md transition-all duration-300 flex flex-col md:flex-row items-start md:items-center gap-5 group`}>
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* Request type */}
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 uppercase tracking-wide ${request.request_type === 'Hospital' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'
              }`}>
        ...
        <div className="shrink-0 w-full md:w-auto flex items-center gap-2">
          <button
            ...
            className="flex items-center justify-center gap-2 w-full md:w-auto px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold rounded-xl transition-all active:scale-95"
          >
            <Navigation size={15} />
            Navigate
          </button>
          ...
          {isResponded ? (
            <div className="px-5 py-2.5 rounded-xl bg-emerald-50 text-emerald-700 text-sm font-bold text-center flex-1 md:flex-none">
              Sent ✓
            </div>
          ) : (
            <button
              onClick={onRespond}
              disabled={responding}
              className="flex items-center justify-center gap-2 w-full md:w-auto px-6 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-60 text-white text-sm font-bold rounded-xl shadow-lg shadow-rose-200 transition-all active:scale-95 flex-1 md:flex-none"
            >
              {responding ? (
